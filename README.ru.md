...
Цвета выбираются не здесь — они приходят из [**ddlc-palette**](https://github.com/rokokol/ddlc-palette), который снимает их с [ddlc.moe](https://ddlc.moe/) (решётка кружков — фоновый тайл 200×200 с самого сайта, вплоть до радиуса). В `theme.conf` они выписаны хексом, чтобы тема по-прежнему ставилась без Nix, а `nix flake check` сверяет одно с другим, так что разъехаться они не могут
